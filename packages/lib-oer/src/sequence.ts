...
export class OerSequence<TShape extends ObjectShape> extends OerType<
  Simplify<InferObjectParseShape<TShape>>,
  Simplify<InferObjectSerializeShape<TShape>>
> {
  private shapeEntries: [string, AnyOerType][]

  constructor(sequenceShape: TShape) {
    super()

    this.shapeEntries = Object.entries(sequenceShape)
  }

  parseWithContext(context: ParseContext, offset: number) {
    const resultObject: Record<string, unknown> = {}
    let totalLength = 0
    for (const [key, oer] of this.shapeEntries) {
